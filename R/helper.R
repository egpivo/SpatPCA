...
#'
#' @keywords internal
#' @param num_cores Optional numeric value representing desired cores. Default is NULL.
#' @return TRUE when `num_cores` passes validation; otherwise NULL
#'
setCores <- function(num_cores = NULL) {
  if (!is.null(num_cores)) {
    ...
    if (length(num_cores) != 1) {
      stop("Please supply a single numeric value for num_cores.")
    }
    default_number <- parallel::detectCores()
    if (is.na(default_number) || default_number < 1) {
      default_number <- 1
    }
